...
```bash
php artisan vendor:publish --tag="filament-table-in-a-schema-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="filament-table-in-a-schema-config"
```

Optionally, you can publish the views using

```bash
php artisan vendor:publish --tag="filament-table-in-a-schema-views"
```
...
```php
$filamentTableInASchema = new DaveMills\FilamentTableInASchema();
echo $filamentTableInASchema->echoPhrase('Hello, DaveMills!');
```
